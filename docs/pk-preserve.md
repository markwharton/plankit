...
- **--notify** *(deprecated)* — Force manual (notify) mode: output a notification about the plan without preserving it. The mode is now read from `.pk.json` (`preserve.mode`). This flag is kept only so an old `pk preserve --notify` hook keeps working until `pk setup` rewrites it bare.
