...
- pk release treats a local-only release branch as a first release (7b5af99)
- site fits small screens and keeps the topic list in view (490618f)

### Maintenance

- make site-serve previews the site, on a phone too (f457973)
...
- release binaries carry the tag's version; the tap gets checksums and its dispatch (6b2df44)
...
- trunk flow releases only from the default branch on origin (dff6439)
...
- report managed files as unchanged when nothing changed (e0ecbaf)
- only rewrite and back up settings.json when it changes (69ce680)
...
- escape & and < in changelog commit subjects (f175f6c)

### Documentation

- rule that all work stays on the source branch (22a5890)
- document release-branch divergence recovery (4d0e2f3)

## [v0.25.0] - 2026-06-24

### Added

- make the update notice install-aware for Homebrew (7e20146)
...
- guard pk changelog against duplicate runs (94cb784)
