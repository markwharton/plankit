...
- Commit types: `feat`, `fix`, `deprecate`, `revert`, `security`, `refactor`, `perf`, `docs`, `chore`, `test`, `build`, `ci`, `style`, `plan` (see `.pk.json`); `plan` is used by pk preserve and hidden in changelog
