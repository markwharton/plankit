...
Skills become much more powerful when they involve the user in the loop. The three skills plankit installs (`.claude/skills/pk-configure/`, `preserve/`, `ship/`) are working examples worth reading.
...
When a skill needs information from the user before it can proceed, ask up front rather than guessing. The `/pk-configure` skill asks about branch policy before writing `.pk.json`:
...
For skills that need to discover state before acting, lay out the steps explicitly. The `/pk-configure` skill detects the git setup, asks its questions, writes `.pk.json`, then offers the working branch with previewed commands. Each step is a distinct instruction in the SKILL.md so Claude follows the sequence reliably.
